...
6. **先讲直觉，再给形式化**：每个核心概念先用类比或具体例子建立直觉（如用"记笔记"类比 SSM vs Transformer），再给出数学定义和公式。
7. **用具体数字和步骤说话**：抽象概念配具体的数字示例（如用「我 喜欢 吃 苹果」逐步演示 KV Cache），让读者能"跟着算一遍"。
8. **按问题特征组织，而非按产品分类**：章节结构围绕"数据有什么特点 → 带来什么技术问题 → 有哪些解法及权衡"展开，不按产品/工具/模型分类罗列。
9. **重设计权衡，轻实现细节**：记录每种方案的核心取舍（如 GQA 的 Pareto 最优点），而非完整的算法流程或代码实现。
10. **善用 Mermaid 图表和对比表**：流程用 Mermaid 可视化，方案对比用精炼的表格（含"权衡"列），避免大段文字叙述。
11. **压缩历史演进，突出核心突破**：技术脉络只需一句话交代前置工作的不足，然后聚焦当前最重要的突破点。
12. **不收录教程代码**：知识文档不放 Python 代码示例或工具使用指南——这些属于实现层面，不是知识层面。
