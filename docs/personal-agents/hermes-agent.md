...
Hermes Agent 的前身是 **OpenClaw**（更早的名称包括 ClawdBot、MoltBot）。2026 年初 Nous Research 将项目重命名为 Hermes Agent 并大幅重构[^hermes-github]，提供了 `hermes claw migrate` 一键迁移工具。

围绕 OpenClaw/Hermes 已形成显著的开源生态：

| 项目 | 说明 |
|------|------|
| **OpenClaw-RL** | 通过对话训练 Agent 的 RL 框架（4.8K stars） |
| **memU** | 24/7 主动式 Agent 的记忆系统（13.3K stars） |
| **Clawith** | OpenClaw 团队协作版（3K stars） |
| **Mission Control** | Agent 编排仪表盘（3.6K stars） |
